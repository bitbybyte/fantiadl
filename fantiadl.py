...
if __name__ == "__main__":
    ...
    dl_group.add_argument("-r", "--prefix-incomplete-posts", action="store_true", dest="prefix_incomplete_posts", help="append a prefix to post directories that are incomplete")
    ...
    try:
        downloader = models.FantiaDownloader(email=email, password=password, dump_metadata=cmdl_opts.dump_metadata, parse_for_external_links=cmdl_opts.parse_for_external_links, autostart_crawljob=cmdl_opts.autostart_crawljob, download_thumb=cmdl_opts.download_thumb, directory=cmdl_opts.output_path, quiet=cmdl_opts.quiet, continue_on_error=cmdl_opts.continue_on_error, use_server_filenames=cmdl_opts.use_server_filenames, prefix_incomplete_posts=cmdl_opts.prefix_incomplete_posts)
